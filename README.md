...
## Планы по масштабированию и оптимизации БД
На текущем этапе архитектура проекта оптимизирована для читаемости и поддержки. Избыточная индексация намеренно не применяется из-за малого объема данных, чтобы избежать оверхеда (замедления операций записи и расхода памяти).
При масштабировании системы до высоких нагрузок (100,000+ записей) планируется следующий стек оптимизаций:

- **Точечное индексирование**: Введение составного индекса по полям `(role, is_active)` в модели `User` для ускорения частых операций фильтрации, а также индексов по`created_at` для временных срезов.
- **Профилирование и мониторинг**: Использование `django-debug-toolbar` и анализа `EXPLAIN` запросов для выявления узких мест и устранения N+1 проблем.
- **Кэширование**: Внедрение Redis для кэширования часто запрашиваемых и редко изменяемых данных (например,  матрицы прав доступа `AccessRoleRule`).
- **Асинхронность**: Вынос тяжелых операций (отправка email, генерация отчетов) в фоновые задачи с помощью Celery
